...
const CreatePin = ({ user }) => {
  ...
  return (
    <div className='flex flex-col justify-center items-center lg:h-4/5'>
      {
        fields && (
          <p className='text-red-500 b-5 text-xl transition-all duration-150 ease-in'>Please fill in all the details</p>
        )
      }
      <div className='flex lg:flex-row flex-col justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full'>
            ...
            { wrongImageType && <p>Wrong Image type</p> }
            {
              !imageAsset ? (
                ...
                <label htmlFor="upload-image">
                  ...
                  <input type="file" id='upload-image' name='upload-image' onChange={uploadImage} className='hidden'/>
                </label>
              ) : (
        ...
        <div className='flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full'>
          ...
          <div className='flex flex-col'>
            <div>
              <p className='mb-2 font-semibold text-lg sm:text'>Choose Pin Category</p>
              <select name="" id=""
              onChange={(e)=>setCategory(e.target.value)}
              className='outline-none w-4/5 text-base border-b-2 border-gray-200 p-2 capitalize rounded-md cursor-pointer'
              >
                <option value="other" className='bg-white sm:text-bg'>Select Category</option>

                {categories.map((catg,key)=>(
                  <option value={catg.name} key={key} className='text-base border-0 outline-none capitalize bg-white text-black'>{catg.name}</option>
                ))}
